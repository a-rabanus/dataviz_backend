"""Trains PyTorch autoencoders and UMAP to reduce clothing and outfit features into 2D latent spaces."""
...
class Autoencoder(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.BatchNorm1d(l2),
            nn.LeakyReLU(0.2),
            nn.Linear(l2, LATENT_DIM)
        )
        
        self.decoder = nn.Sequential(
            nn.Linear(LATENT_DIM, l2),
            nn.LeakyReLU(0.2),
            nn.Linear(l2, l1),
            nn.LeakyReLU(0.2),
...
def generate_matrices(df, mode="item"):
    ...
    if mode == "item":
        ...
        base_features[:, 0:3] *= 2.0
        base_features[:, 3] *= 2.0
        base_features[:, 4] *= 0.5

        loc_features = df.select(['loc_x', 'loc_y', 'loc_z']).to_numpy() * 0.1
        time_features = df.select(['time_month_sin', 'time_month_cos', 'time_day_sin', 'time_day_cos', 'time_hour_sin', 'time_hour_cos']).to_numpy() * 0.1
        ...
        color_idx = [i for i, c in enumerate(target_cols) if c.startswith('dist_color_')]
        tex_idx = [i for i, c in enumerate(target_cols) if c.startswith('dist_tex_')]
        area_idx = [i for i, c in enumerate(target_cols) if c.startswith('dist_area_')]
        
        base_features[:, color_idx] *= 2.0
        base_features[:, tex_idx] *= 2.0
        base_features[:, area_idx] *= 0.5
        ...
        loc_features = meta.select(['loc_x', 'loc_y', 'loc_z']).to_numpy() * 0.1
        time_features = meta.select(['time_month_sin', 'time_month_cos', 'time_day_sin', 'time_day_cos', 'time_hour_sin', 'time_hour_cos']).to_numpy() * 0.1
    ...
    for suffix, matrix in configs:
        print(f"Processing {prefix}_{suffix}...")
